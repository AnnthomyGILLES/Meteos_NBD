...
def test_get_weather_data_success(monkeypatch, api_call_info):
    ...
    mock_response.json.return_value = {"cod": 200, "weather": [{"description": "clear sky"}],
                                       "main": {"temp": 15.0, "humidity": 60}, "wind": {"speed": 2.0}}
    monkeypatch.setattr(requests, "get", Mock(return_value=mock_response))

    # Call the function and check the result
    result = get_weather_data(api_call_info['URL'])
    assert result == {"cod": 200, "weather": [{"description": "clear sky"}], "main": {"temp": 15.0, "humidity": 60},
                      "wind": {"speed": 2.0}}
